...
export const useNavigation = (telegram) => {
  const [currentPage, setCurrentPage] = useState(() => {
    ...
    if (isAdminRoute) {
      console.log('🔓 [useNavigation] Admin route detected, allowing access');
      return ROUTES.ADMIN;
    }
    
    console.log('🔒 [useNavigation] Default to MAIN page');
    return ROUTES.MAIN;
  });
  ...
  useEffect(() => {
    const handleBackButton = () => {
      // Don't navigate away from admin panel when back button is pressed
